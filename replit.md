...
**✅ Universal Prompt v1.1 Final (Agent3 Router - CEO Approved) - Production Ready (October 28, 2025)**: Complete ultra-compact universal prompt with CEO directive and success criteria deployed. **CEO Mandate**: Agent3 must detect active overlay, operate within boundaries, emit required events, meet SLOs, uphold finance rules (4x AI markup + 3% provider fee drive profit). **Agent3 Router**: Production-ready docs/system-prompts/universal.prompt (2,688 bytes merged, 5,383 bytes raw) with CEO_DIRECTIVE.md validation guide and **Purpose** section using sections (A-H): (Purpose) Single system prompt across 8 Scholar AI Advisor apps - detect overlay, lock to it, operate only within its rules; (A) Routing & Isolation - first-match-wins detection, immediate overlay_selected emission, strict isolation ("do not call actions, read policies, or leak details from non-selected overlays"); (B) Company Core - market leadership mission, dual-engine growth (B2C+B2B), SEO-first via Auto Page Maker, data-first principles; (C) Global Guardrails - no essays/PII/secrets, server-side trust boundary for revenue, FERPA/COPPA compliance; (D) KPI & Telemetry - "emit exactly as specified" with overlay_selected, revenue events (credit_purchase_succeeded, fee_accrued with "fee_usd must never be computed client-side or in-agent"), slo_at_risk, error, kpi_brief_generated; (E) SLOs - uptime ≥99.9%, P95 ≤120ms, escalate if p95_ms>150 for 5+ min or event drop >10%; (F) 8 App Overlays with Allowed/Must not/Required; (G) Operating Procedure - Plan→Execute→Validate→Report→Escalate; (H) Definition of Done - objective achieved, events emitted, SLOs met, "finance rules upheld (4x markup; 3% fee server-side only)". **Success Criteria**: (1) Telemetry ✅ overlay_selected every session, revenue complete, no PII, server-side fees; (2) SLOs ⏱️ 99.9% uptime, P95≤120ms; (3) Finance 💰 4x markup + 3% fee positive unit economics; (4) Governance ✅ zero incidents; (5) ARR 📊 non-zero arr_usd + fee_revenue_usd by T+72h. **All 8 Overlays Verified**: executive_command_center (KPI briefs, kpi_brief_generated), auto_page_maker (SEO pages, overlay_selected), student_pilot (B2C "ensure pricing ≥ 4x AI cost", credit_purchase_succeeded), provider_register (B2B "route to server for fee calculation", fee_accrued server-side only), scholarship_api (default/fallback, overlay_selected), scholarship_agent (marketing briefs, overlay_selected), scholar_auth (OAuth best practices, overlay_selected), scholarship_sage (policy/compliance Q&A, overlay_selected). **Rollout**: T+0 ✅ CEO-approved with Purpose section; T+24h scholarship_api/agent; T+48h student_pilot/provider_register revenue validation; T+72h first kpi_brief_generated.
