...
**✅ Universal Prompt v1.1 Final (Agent3 Router - CEO Approved) - Production Ready (October 28, 2025)**: Complete ultra-compact universal prompt with CEO directive and success criteria deployed. **CEO Mandate**: Agent3 must detect active overlay, operate within boundaries, emit required events, meet SLOs, uphold finance rules (4x AI markup + 3% provider fee drive profit). **Agent3 Router**: Production-ready docs/system-prompts/universal.prompt (2,015 bytes merged, 4,343 bytes raw) with CEO_DIRECTIVE.md validation guide using sections (A-H): (A) Routing & Isolation - "on start" first-match-wins detection, immediate overlay_selected emission, strict isolation; (B) Company Core - market leadership mission, dual-engine growth (B2C+B2B), SEO-first via Auto Page Maker, data-first principles; (C) Global Guardrails - no essays/PII/secrets, server-side revenue math, FERPA/COPPA compliance; (D) KPI & Telemetry - bootstrap overlay_selected, revenue events (credit_purchase_succeeded, fee_accrued with server-side fee only), slo_at_risk, error; (E) SLOs - uptime ≥99.9%, P95 ≤120ms, escalate if p95>150ms for 5+ min or event drop >10%; (F) 8 App Overlays with Allowed/Must not/Required events; (G) Operating Procedure - Plan→Execute→Validate→Report→Escalate; (H) Definition of Done - objective achieved, events emitted, SLOs met, no fee_usd in-agent. **Success Criteria**: (1) Telemetry ✅ overlay_selected every session, revenue complete, no PII, server-side fees; (2) SLOs ⏱️ 99.9% uptime, P95≤120ms; (3) Finance 💰 4x markup + 3% fee positive unit economics; (4) Governance ✅ zero incidents; (5) ARR 📊 non-zero arr_usd + fee_revenue_usd by T+72h. **All 8 Overlays Verified**: executive_command_center (KPI briefs), auto_page_maker (SEO pages, page_plan_created+page_published), student_pilot (B2C no essays, credit_purchase_succeeded), provider_register (B2B server-side fee only, fee_accrued), scholarship_api (docs, api_doc_viewed), scholarship_agent (campaigns, experiment_defined+campaign_plan_created), scholar_auth (auth flows, auth_doc_viewed), scholarship_sage (policy Q&A, guidance_provided). **Rollout**: T+0 ✅ CEO-approved; T+24h scholarship_api/agent; T+48h student_pilot/provider_register revenue validation; T+72h first kpi_brief_generated.
